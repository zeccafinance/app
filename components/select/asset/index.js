...
export default ({
  ...
  const { preferences, chains, assets, pool_assets } = useSelector(
    (state) => ({
  ...
  return (
    <Modal
      ...
      buttonTitle={
        _assets_data ? (
          <div
            className={
              fixed
                ? "cursor-default flex items-center rounded-2xl space-x-1.5 sm:space-x-2 sm:-ml-1"
                : className ||
                  "w-32 sm:w-48 min-w-max bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 rounded-2xl border dark:border-slate-700 flex items-center justify-between space-x-1.5 sm:space-x-2 py-1.5 sm:py-2 px-2 sm:px-3"
            }
          >
            {image && (
              <>
            ...
            <span
              className={`whitespace-nowrap sm:text-lg font-semibold ${
                !image ? "sm:ml-3" : ""
              }`}
            >
              {symbol}
      ...
      buttonClassName={
        className ||
        `w-32 sm:w-48 min-w-max h-10 sm:h-12 ${
          disabled ? "cursor-not-allowed" : ""
        } flex items-center justify-center`
      }
      title={
        <div className="flex items-center justify-between pt-1 pb-2 space-x-2">
