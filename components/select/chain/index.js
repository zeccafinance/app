...
export default ({
  ...
  const boxShadow =
    color &&
    !noShadow &&
    `${color}${theme === "light" ? "44" : "33"} 0px 4px 16px 8px`;
  ...
  return (
    <Modal
          ...
          <div
            className={
              fixed
                ...
                : className ||
                  "w-32 sm:w-40 rounded-xl min-w-max bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 rounded border dark:border-slate-700 flex items-center justify-between space-x-0.5 sm:space-x-2 py-1.5 sm:py-2 pl-2 pr-1 sm:px-1.5"
            }
          >
            <div className="flex items-center space-x-2">
              ...
              <span className="font-semibold whitespace-nowrap sm:text-lg">
      ...
      buttonClassName={
        className ||
        `w-32 sm:w-40 min-w-max h-8 sm:h-10 ${
      ...
      title={
        <span className="flex items-center pt-1 pb-2 space-x-1">
          <span className="capitalize">{origin || "select"}</span>
          <span className="normal-case">chain</span>
        </span>
      }
      body={
        <Search
