...
export default ({
  ...
  const { preferences, assets, rpc_providers, wallet, balances } = useSelector(
    (state) => ({
  ...
  return (
    ...
    asset && (
      <div
        className={`flex items-center justify-center text-slate-600 dark:text-slate-50 text-sm space-x-1 ${className}`}
      >
        {["string", "number"].includes(typeof amount) && !isNaN(amount) ? (
          <>
            <DecimalsFormat value={amount} className="font-semibold" />
            {!hideSymbol && (
              <span className="hidden font-semibold sm:block">{symbol}</span>
            )}
          </>
          ...
          signer && (
            <RotatingSquare width="16" height="16" color={loaderColor(theme)} />
          )
        )}
      </div>
    )
