...
export default () => {
  ...
  useEffect(() => {
    ...
    if (!(params.chain && params.asset) && pool_assets_data?.length > 0) {
      ...
      params.chain =
        params.chain || getChain(_.head(contracts)?.chain_id, chains_data)?.id;
    ...
    const getData = async () => {
      ...
      if (sdk && address && chain && poolsTrigger) {
        ...
        try {
          ...
          console.log("[getUserPools error]", {
            domain_id,
            address,
            error,
          });
  ...
  const no_pool =
    selected && !getAsset(asset, pool_assets_data, chain_data?.chain_id);
    ...
    <div className="mb-4">
      <div className="flex justify-center">
        <div className="w-full flex flex-col space-y-3.5 my-4 sm:my-12 mx-1 sm:mx-4">
          <Link href="/pools">
            <div className="w-fit rounded-xl  dark:bg-[#112727] flex items-center text-slate-600 dark:text-slate-500 space-x-1 py-0.5 px-2.5">
              <TiArrowLeft size={18} className="-ml-0.5" />
              <span className="text-base font-semibold">Back to Pools</span>
            </div>
          </Link>
              ...
              <div className="flex flex-wrap items-center justify-between space-y-4 lg:space-y-0 sm:space-x-2">
                <div className="flex items-center space-x-1">
                  <div className="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <span className="text-xl font-semibold tracking-tighter sm:text-5xl">
                      <span className="mr-1">{chainName(chain_data)}</span>
                ...
                >
                  <div>
                    <span className="text-base font-medium text-slate-400 dark:text-slate-200">
                      Reward APY
                    </span>
