...
export default ({ data }) => {
  const { preferences, chains, assets, wallet } = useSelector(
    (state) => ({
  ...
  const errored =
    error_status &&
    !execute_transaction_hash &&
    [XTransferStatus.XCalled, XTransferStatus.Reconciled].includes(status);

  return (
    transferData && (
      <div
        className={`bg-slate-100 dark:bg-slate-900 max-w-xs sm:max-w-none rounded ${
          errored
      ...
      >
            ...
            <div className="flex items-center justify-center">
              ...
              <Fade left distance="64px" duration={2500} forever>
                <div
                  ...
                  style={{
                    background: `${
                      source_asset_data?.color || loaderColor(theme)
                    }aa`,
                  }}
                />
              </Fade>
        ...
        <div className="flex items-start justify-between my-4 space-x-2">
          <div className="flex flex-col space-y-1.5">
            {typeof source_amount === "number" && (
              <DecimalsFormat value={source_amount} />
            )}
            <div className="flex items-center justify-start space-x-1">
              ...
              <span className="text-xs font-medium">{source_symbol}</span>
            </div>
          </div>
          ...
          >
            {typeof destination_amount === "number" && (
              <DecimalsFormat value={destination_amount} />
            )}
            <div className="flex items-center justify-center space-x-1">
              ...
              <span className="text-xs font-medium">{destination_symbol}</span>
            </div>
          </div>
        </div>
            ...
            <EnsProfile
              ...
              fallback={
                <Copy
                  value={to}
                  title={
                    <span className="text-sm cursor-pointer text-slate-600 dark:text-white">
                      <span className="sm:hidden">{ellipse(to, 12)}</span>
                      <span className="hidden sm:block">{ellipse(to, 8)}</span>
                    </span>
                  }
                />
              }
              ...
              <ActionRequired
                ...
                buttonTitle={
                  <Tooltip
                    ...
                    className="z-50 text-xs text-white bg-dark"
                  >
                    <span className="text-xs font-semibold text-red-600 whitespace-nowrap dark:text-red-500">
                      {error_status}
                    </span>
                  </Tooltip>
                }
            ...
            >
              <div className="flex items-center">
                <span className="text-xs font-medium text-slate-400 dark:text-slate-500">
                  {moment(xcall_timestamp * 1000).format(
        ...
        {transfer_id && (
          <div className="flex items-center justify-end mt-1 -mb-2">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center -mr-1 space-x-0 text-xs font-medium text-blue-500 dark:text-blue-500"
            >
