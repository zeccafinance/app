...
export default () => {
      ...
      if (page_visible && assets_data) {
        ...
        if (updated_ids.length < assets_data.length) {
          ...
          if (assets.length > 0) {
            ...
            response.forEach((d) => {
              const index = assets_data.findIndex((a) =>
                equalsIgnoreCase(a.id, d?.asset_id)
              );
        ...
        const pool_assets_data = getAsset(
          null,
          assets_data,
          undefined,
          undefined,
        ...
        if (
          pool_assets_data.findIndex((d) => !updated_ids.includes(d?.id)) < 0
        ) {
          dispatch({
            type: POOL_ASSETS_DATA,
  ...
  useEffect(() => {
    const getData = async (is_interval) => {
      if (page_visible && chains_data) {
        const updated_ids = toArray(gas_tokens_price_data)
          .filter((d) => !is_interval && typeof d.price === "number")
          .map((d) => d.asset_id);

        const gas_tokens = toArray(
          chains_data.map(
            (c) => _.head(c.provider_params)?.nativeCurrency?.symbol
          ),
          "lower"
        );
        ...
        if (updated_ids.length < gas_tokens.length) {
          ...
          if (assets.length > 0) {
            ...
            if (data) {
              response.forEach((d) => {
                const index = toArray(data).findIndex(
                  (_d) => _d?.asset_id === d?.asset_id
                );
  ...
  // rpcs
  useEffect(() => {
    const createRpcProvider = (url, chain_id) =>
  ...
  useEffect(() => {
    const init = async () => {
      if (
        ...
        for (const chain_data of chains_data) {
          const { disabled, chain_id, domain_id, provider_params } = {
            ...chain_data,
          };
          let { rpc_urls } = { ...chain_data };
    ...
    const getData = async () => {
      if (
        ...
        getChain(null, chains_data, true, false, false, undefined, true)
          .length <= Object.keys(router_asset_balances_data).length
      ) {
  ...
  useEffect(() => {
      ...
      if (contract_address) {
        ...
        try {
          ...
          if (
            ...
            try {
              stats = _.cloneDeep(
                await sdk.sdkPool.getYieldData(
                  domain_id,
                  contract_address,
                  number_of_days
                )
    ...
    const getChainData = async (chain_data) =>
      pool_assets_data.forEach((a) => getPoolData(chain_data, a));
      ...
      if (id) {
        ...
        try {
          ...
          const response = _.cloneDeep(
            await sdk.sdkPool.getUserPools(domain_id, address)
          );
              ...
              _.concat(
                ...
                response.map((p) => {
                  ...
                  return {
                    ...
                    poolTokenBalances: toArray(poolTokenBalances).map((b, i) =>
                      Number(
                        utils.formatUnits(
                          BigInt(b || "0"),
                          adopted?.index === i
  ...
  useEffect(() => {
    ...
    const getData = async (is_interval) => {
      if (
        ...
        const all_chains_data = getChain(
          null,
          chains_data,
          true,
          false,
        ...
        const data =
          get_balances_data &&
          !is_interval &&
          all_chains_data.findIndex((c) => !balances_data?.[c.chain_id]) < 0
            ? Array.isArray(get_balances_data)
              ? get_balances_data
              : [get_balances_data]
            : all_chains_data.map((c) => {
                return {
                  chain: c.id,
                };
              });
        ...
        data.forEach((c) => {
          ...
          if (contract_data) {
            ...
            getChainContracts(chain_id, assets_data).forEach((c) =>
              getMyBalance(chain_id, c)
            );
          }
        });
      }
  ...
  return (
    <>
      <div className="navbar">
        <div className="flex items-center justify-between w-full gap-4 border-2 border-[#11191F] navbar-inner sm:h-20 xl:grid xl:grid-flow-row xl:grid-cols-3">
          ...
          <div className="flex items-center justify-end">
            {browser_provider && <Chains chain_id={chain_id} />}
      ...
      {status_message && (
        <div className="flex items-center w-full px-2 py-2 overflow-x-auto bg-slate-100 dark:bg-slate-800 dark:bg-opacity-50 sm:py-3 sm:px-4">
          <div className="flex flex-wrap items-center text-green-600 dark:text-green-400 text-2xs xl:text-sm font-bold space-x-1.5 xl:space-x-2 mx-auto">
            <span className="status-message">
              <Linkify>{parse(status_message)}</Linkify>
            </span>
          </div>
        </div>
      )}
