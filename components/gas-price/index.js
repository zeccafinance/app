...
export default ({ chainId, dummy, iconSize = 20, className = "" }) => {
  const { preferences, rpc_providers } = useSelector(
    (state) => ({
  ...
  return chainId ? (
    <div
      className={`flex items-center justify-center text-slate-400 dark:text-slate-500 space-x-1 ${className}`}
    >
